...
Run locally with:  streamlit run st_app.py   (from this "app/" directory)
Or deployed on Streamlit Community Cloud, main file path: app/st_app.py

File paths below are anchored to this script's own location (not the
current working directory), so this runs correctly either way.
"""
...
BASE_DIR = Path(__file__).resolve().parent.parent   # project root, regardless of launch location
